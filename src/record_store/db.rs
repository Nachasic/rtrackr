use super::{
    utils::{Database, RecordStoreError},
    ActivityRecord,
};
...
impl DB {
        ...
        F: FnOnce(&Vec<ActivityRecord>),
    {
        ...
        self.db.read(|store| match store.get(&date_record.0) {
            Some(data) => {
                f(data);
                Ok({})
            }
    ...
    where
        F: FnOnce(&mut Vec<ActivityRecord>),
    {
        let date_record = TrackingDate::from(date);
